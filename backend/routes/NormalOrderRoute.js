...
router.post('/', async (req, res) => {
  try {
    const { cartItems, customerName, customerNumber, customerAddress, customerPincode, totalAmount } = req.body;

    // Validate required fields
    if (!Array.isArray(cartItems) || cartItems.length === 0 || !customerName || !customerNumber || !customerAddress || !customerPincode || totalAmount === undefined) {
    ...
    // Validate and convert customerNumber
    const formattedCustomerPincode = parseInt(customerPincode, 6);
    if (isNaN(formattedCustomerPincode)) {
      return res.status(400).json({ success: false, message: 'Customer pincode must be valid' });
    }
    ...
    const newOrder = new Order({
      ...
      customerPincode: formattedCustomerPincode, // Save as pincode
      cartItems,
      totalAmount
    });
