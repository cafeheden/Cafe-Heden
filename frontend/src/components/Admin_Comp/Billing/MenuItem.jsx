...
const MenuItem = ({ item }) => {
  ...
  const handleAddToCart = () => {
    if (isLoggedIn) {
      ...
      handleError("You need to be logged in to add items to the cart.");
    }
  };
      ...
      <div className="flex justify-between items-center mt-auto">
        ...
        <button
          ...
          className={`px-4 py-1 border-2 ${
            isLoggedIn
              ? "border-primaryColor text-primaryColor hover:bg-primaryColor hover:text-tertiaryColor"
              : "border-lightGrey text-gray-400 cursor-not-allowed"
          }`}
          disabled={!isLoggedIn} // Disables the button if not logged in
        >
          Add Item
        </button>
      </div>
